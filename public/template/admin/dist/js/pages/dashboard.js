    ...
    fetch(route)
        ...
        .then((responseData) => {
            ...
            responseData.forEach((item) => {
              var itemUpdatedAt = new Date(item.updated_at).toISOString().split("T")[0];
              var itemCreatedAt = new Date(item.created_at).toISOString().split("T")[0];
              if (labels.includes(itemUpdatedAt)) {
                  chartData.push(item.total_amount);
              }
          
              if (labels.includes(itemCreatedAt)) {
                  chartData.push(item.total_amount);
              }
